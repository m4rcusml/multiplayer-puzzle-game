...
export function clampGroupDelta(pieces, movingGroupId, dx, dy) {
...
export function overlapsOtherGroup(pieces, movingGroupId, dx, dy) {
    const moving = groupPieces(pieces, movingGroupId);
...
export function snapGroupToNeighbor(pieces, pieceId) {
    ...
    const neighbor = pieces.find(other => {
        if (other.id === piece.id || (other.groupId ?? other.id) === movingGroupId)
            return false;
    ...
    return pieces.map(item => {
        ...
        if (!inMovingGroup && !inNeighborGroup)
            return item;
        return {
            ...item,
...
//# sourceMappingURL=room-geometry.js.map